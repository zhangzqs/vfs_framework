...
class UnionFileSystem extends IFileSystem with FileSystemHelper {
  final List<UnionFileSystemItem> _items;

  UnionFileSystem(List<UnionFileSystemItem> items) 
    : _items = List.from(items)..sort((a, b) => b.priority.compareTo(a.priority));
  ...
  Future<UnionFileSystemItem?> _getFirstReadableItemAsync(Path path) async {
    ...
    items.sort((a, b) {
      final mountLengthCompare = b.mountPath.segments.length.compareTo(a.mountPath.segments.length);
  ...
  UnionFileSystemItem? _getFirstWritableItem(Path path) {
    final items = _getItemsForPath(path).where((item) => !item.readOnly).toList();
  ...
  Future<void> copy(
    ...
    if (destItem == null) {
      throw FileSystemException.permissionDenied(destination);
    }
    ...
    if (sourceStatus.isDirectory) {
      ...
      if (options.recursive) {
        await for (final item in sourceItem.fileSystem.list(sourceInternalPath, options: const ListOptions(recursive: true))) {
          final relativePath = Path(item.path.segments.skip(sourceInternalPath.segments.length).toList());
          final srcPath = Path([...source.segments, ...relativePath.segments]);
          final dstPath = Path([...destination.segments, ...relativePath.segments]);
      ...
      await destItem.fileSystem.writeBytes(
        destInternalPath,
        data,
        options: WriteOptions(mode: options.overwrite ? WriteMode.overwrite : WriteMode.write),
      );
    }
  }
  ...
  Stream<FileStatus> list(
    ...
    for (final item in items) {
      ...
      try {
        await for (final status in item.fileSystem.list(internalPath, options: options)) {
          // 将内部路径转换回union路径
          final unionPath = Path([...path.segments, ...status.path.segments.skip(internalPath.segments.length)]);
  ...
  Future<void> move(
    ...
    await copy(
      source,
      destination,
      options: CopyOptions(overwrite: options.overwrite, recursive: options.recursive),
    );
