...
void main() async {
  ...
  await fs1.writeBytes(Path(['subdir', 'file2.txt']), Uint8List.fromList([4, 5, 6]));
  ...
  await for (final item in unionFs.list(Path.rootPath)) {
    print('  - ${item.path}: ${item.isDirectory ? "目录" : "文件"} (${item.size} bytes)');
  }
  
  print('\n测试PROPFIND请求:');
  final request = Request('PROPFIND', Uri.parse('http://localhost:8080/'), headers: {
    'depth': '1',
  });
  ...
  print('\n测试/data目录PROPFIND:');
  final dataRequest = Request('PROPFIND', Uri.parse('http://localhost:8080/data'), headers: {
    'depth': '1',
  });
