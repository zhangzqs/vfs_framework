...
void main() {
  group('HttpServer Tests', () {
    ...
    test('应该能以JSON格式列举目录', () async {
      final request = Request('GET', Uri.parse('http://localhost:8080/documents'),
    ...
    test('应该能递归列举目录', () async {
      final request = Request('GET', Uri.parse('http://localhost:8080/documents?recursive=true'),
          headers: {'accept': 'application/json'});
      final response = await httpServer.handleRequest(request);
    ...
    test('应该能下载文件', () async {
      final request = Request('GET', Uri.parse('http://localhost:8080/documents/test.txt'));
      final response = await httpServer.handleRequest(request);
    ...
    test('应该返回404当文件不存在', () async {
      final request = Request('GET', Uri.parse('http://localhost:8080/nonexistent.txt'));
      final response = await httpServer.handleRequest(request);
    ...
    test('应该返回400当尝试下载目录', () async {
      final request = Request('GET', Uri.parse('http://localhost:8080/documents?list=false'),
          headers: {'accept': 'application/json'});
